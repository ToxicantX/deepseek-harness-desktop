...
当前客户端与请求转换点对应 DSH `0.1.1-rc.2`。升级 Runtime 后需要重新运行专项测试和真实 Electron 请求烟测；若上游调整设置模块或请求头组装结构，转换会停止命中并保留上游原行为。转换后的客户端工厂在执行阶段抛错时，共享 Loader 会回退到 DSH 原始工厂，避免设置模块故障扩散到窗口启动和基础对话能力。
