...
$GoalGuardPluginSource = Join-Path $RepositoryRoot 'runtime/goal-no-progress-guard-plugin'
...
$GoalGuardPlugin = Join-Path $App 'plugins/goal-no-progress-guard'
...
if (-not (Test-Path (Join-Path $GoalGuardPluginSource 'index.js'))) { throw 'Goal no-progress guard runtime plugin is incomplete; index.js is missing.' }
if (-not (Test-Path (Join-Path $GoalGuardPluginSource 'package.json'))) { throw 'Goal no-progress guard runtime plugin is incomplete; package.json is missing.' }
New-Item $RepairPlugin -ItemType Directory -Force | Out-Null
New-Item $PetBridgePlugin -ItemType Directory -Force | Out-Null
New-Item $GoalGuardPlugin -ItemType Directory -Force | Out-Null
...
Copy-Item (Join-Path $GoalGuardPluginSource '*') $GoalGuardPlugin -Recurse -Force
...
@"
{
  ...
  "dependencies": {
    ...
    "@deepseek-ai/dsh-desktop-goal-no-progress-guard": "file:./plugins/goal-no-progress-guard"
  }
}
...
try {
  ...
  node -e 'const fs=require("node:fs"); const path=process.argv[1]; const value=JSON.parse(fs.readFileSync(path, "utf8")); value.dependencies ??= {}; value.dependencies["@deepseek-ai/dsh-desktop-session-repair"] = "0.1.0"; value.dependencies["@deepseek-ai/dsh-desktop-pet-bridge"] = "0.1.0"; value.dependencies["@deepseek-ai/dsh-desktop-goal-no-progress-guard"] = "0.1.0"; fs.writeFileSync(path, JSON.stringify(value, null, 2) + "\n")' $DshManifest
