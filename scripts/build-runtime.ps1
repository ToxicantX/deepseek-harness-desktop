...
try {
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Upstream DSH runtime workspace dependency normalization failed.' }
} finally {
  Pop-Location
}
...
if (-not (Test-Path $DshManifest)) { throw 'Deployed DSH package manifest is missing from standalone node_modules.' }
