...
describe('plugin manager UI contract', () => {
  ...
  it('keeps all plugin UI behavior in the sandboxed preload without HTML sinks', () => {
    ...
    expect(preload).toContain('const completedUpdates = new Set<string>()')
    expect(preload).toContain('if (!completedUpdates.has(entry.name) && (latestVersion !== undefined || entry.manualUpdate === true))')
    expect(preload).toContain("if (input.action === 'update') completedUpdates.add(input.packageName)")
    expect(preload).toContain("result.action === 'update' && result.packageName !== undefined")
    expect(preload).toContain('completedUpdates.clear()')
    expect(preload).toContain("latestVersion === undefined ? '更新' : '更新至 ' + latestVersion")
    expect(preload).not.toContain('if (entry.spec !== undefined)')
    expect(preload).toContain('entry.name === update.name && entry.version === update.currentVersion')
