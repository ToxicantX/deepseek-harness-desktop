...
describe('desktop shell conversation replay injector', () => {
  ...
  it('ships the injector in preload and removes the Runtime plugin wiring', () => {
    ...
    const scopeCreationIndex = build.indexOf("New-Item (Join-Path $App 'node_modules/@deepseek-ai') -ItemType Directory -Force")
    const moveIndex = build.indexOf('Move-Item $DshPackage $DeployedDsh -Force')
    const recursiveDshCopyIndex = build.indexOf("Copy-Item (Join-Path $DshPackage '*') $App -Recurse -Force")
    const copiedManifestValidationIndex = build.indexOf("$CopiedDshManifest = Join-Path $DeployedDsh 'package.json'")
    ...
    expect(scopeCreationIndex).toBeGreaterThan(normalizationIndex)
    expect(moveIndex).toBeGreaterThan(scopeCreationIndex)
    expect(recursiveDshCopyIndex).toBe(-1)
    expect(copiedManifestValidationIndex).toBeGreaterThan(moveIndex)
