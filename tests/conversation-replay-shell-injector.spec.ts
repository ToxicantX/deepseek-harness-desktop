...
describe('desktop shell conversation replay injector', () => {
  ...
  it('ships the injector in preload and removes the Runtime plugin wiring', () => {
    ...
    const copyIndex = build.indexOf("Copy-Item (Join-Path $DshPackage '*') $App -Recurse -Force")
    const copiedManifestValidationIndex = build.indexOf("$CopiedDshManifest = Join-Path $App 'package.json'")
    ...
    expect(copyIndex).toBeGreaterThan(normalizationIndex)
    expect(copiedManifestValidationIndex).toBeGreaterThan(copyIndex)
