...
describe('global personalization UI contract', () => {
  it('ships a script-free local Markdown editor with stable controls', () => {
    ...
    expect(html).toContain('个性化设置')
    expect(html).not.toContain('个人化')
  ...
  it('edits only through preload DOM APIs with byte limits and dirty tracking', () => {
    ...
    expect(preload).not.toContain('个人化')
  ...
  it('opens from File menu and scopes IPC to its exact utility window', () => {
    ...
    expect(main).toContain("label: '个性化设置...'")
    ...
    expect(main).toContain("message: '个性化设置有尚未保存的更改。'")
    ...
    expect(beforeQuit.indexOf('if (personalizationDirty && personalizationWindow')).toBeLessThan(beforeQuit.indexOf('if (controller === undefined && petWindow === undefined)'))
  })
})
