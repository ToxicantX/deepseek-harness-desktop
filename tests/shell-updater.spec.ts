...
describe('ShellUpdater', () => {
  ...
  it('disables console logging in the packaged GUI process', () => { new ShellUpdater({} as any, vi.fn(async () => {})); expect(updater.logger).toBeNull() })
  ...
  it('stops runtime before silently installing after one confirmation', async () => { const order: string[] = []; const stop = vi.fn(async () => { order.push('stop') }); updater.quitAndInstall.mockImplementation(() => { order.push('install') }); await new ShellUpdater({} as any, stop).check(true); expect(stop).toHaveBeenCalledOnce(); expect(updater.quitAndInstall).toHaveBeenCalledWith(true, true); expect(order).toEqual(['stop', 'install']); expect(dialog.showMessageBox).toHaveBeenCalledTimes(1) })
  it('does not download, stop, or install when the user declines', async () => { dialog.showMessageBox.mockResolvedValue({ response: 1 }); const stop = vi.fn(async () => {}); await new ShellUpdater({} as any, stop).check(true); expect(updater.downloadUpdate).not.toHaveBeenCalled(); expect(stop).not.toHaveBeenCalled(); expect(updater.quitAndInstall).not.toHaveBeenCalled() })
  it('cleans up listeners and never installs on download failure', async () => { updater.downloadUpdate.mockRejectedValue(new Error('failed')); const stop = vi.fn(async () => {}); await new ShellUpdater({} as any, stop).check(false); expect(updater.listenerCount('download-progress')).toBe(0); expect(stop).not.toHaveBeenCalled(); expect(updater.quitAndInstall).not.toHaveBeenCalled(); expect(dialog.showMessageBox).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ type: 'error' })) })
  it('never installs when stopping Runtime fails', async () => { const stop = vi.fn(async () => { throw new Error('stop failed') }); await new ShellUpdater({} as any, stop).check(false); expect(stop).toHaveBeenCalledOnce(); expect(updater.quitAndInstall).not.toHaveBeenCalled(); expect(dialog.showMessageBox).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ type: 'error' })) })
