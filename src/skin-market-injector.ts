type SkinLocaleDictionary = Record<string,string>
type SkinLocaleParams = Record<string,string|number>

export function createSkinLocaleAdapter(initialLocale:string){
  let activeLocale=String(initialLocale||'en')
  const listeners=new Set<()=>void>()
  const dictionaries=new Map<string,Map<string,SkinLocaleDictionary>>()
  const translators=new Map<string,(key:string,params?:SkinLocaleParams)=>string>()
  const localeKeys=()=>{const normalized=activeLocale.replaceAll('_','-').toLowerCase();const primary=normalized.split('-',1)[0]||'en';return [...new Set([normalized,primary,'en'])]}
  const translate=(namespace:string,key:string,params?:SkinLocaleParams)=>{const group=dictionaries.get(namespace);let value:string|undefined;for(const locale of localeKeys()){value=group?.get(locale)?.[key];if(value!==undefined)break}if(value===undefined)value=key;return value.replace(/\{(\w+)\}/g,(match,name)=>params?.[name]===undefined?match:String(params[name]))}
  return {
    getLocale:()=>({active:activeLocale}),
    subscribe(listener:()=>void){listeners.add(listener);return()=>listeners.delete(listener)},
    register(namespace:string,localeOrDictionaries:string|Record<string,SkinLocaleDictionary>,dictionary?:SkinLocaleDictionary){
      const name=String(namespace);let group=dictionaries.get(name);if(group===undefined){group=new Map();dictionaries.set(name,group)}
      const entries=typeof localeOrDictionaries==='string'?[[localeOrDictionaries,dictionary??{}] as const]:Object.entries(localeOrDictionaries??{})
      const previous=new Map<string,SkinLocaleDictionary|undefined>()
      for(const [locale,value] of entries){const key=String(locale).replaceAll('_','-').toLowerCase();previous.set(key,group.get(key));group.set(key,value)}
      listeners.forEach(listener=>listener())
      return()=>{const current=dictionaries.get(name);if(current===undefined)return;for(const [locale,value] of previous){if(value===undefined)current.delete(locale);else current.set(locale,value)}if(current.size===0)dictionaries.delete(name);listeners.forEach(listener=>listener())}
    },
    bind(namespace:string){const name=String(namespace);let translator=translators.get(name);if(translator===undefined){translator=(key,params)=>translate(name,String(key),params);translators.set(name,translator)}return translator},
    t:(key:string,params?:SkinLocaleParams)=>translate('',String(key),params),
  }
}

export function createSkinSessionsAdapter(){
  type Snapshot={current?:string;byId:Record<string,{running:boolean;pendingInteraction?:unknown}>}
  let snapshot:Snapshot={byId:{}}
  const listeners=new Set<()=>void>()
  const bindings=new Map<string,{session:{getSnapshot:()=>{lastAgentError:null};subscribe:(listener:()=>void)=>()=>void}}>()
  const list={getSnapshot:()=>snapshot,subscribe(listener:()=>void){listeners.add(listener);return()=>listeners.delete(listener)}}
  return {
    list,
    binding(id:string){const key=String(id);let binding=bindings.get(key);if(binding===undefined){binding={session:{getSnapshot:()=>({lastAgentError:null}),subscribe:()=>()=>{}}};bindings.set(key,binding)}return binding},
    setCurrent(id:string|null|undefined){const current=typeof id==='string'&&id.length>0?id:undefined;if(snapshot.current===current)return;snapshot=current===undefined?{byId:{}}:{current,byId:{[current]:{running:false}}};listeners.forEach(listener=>listener())},
  }
}
...
export function createClientBundleAdapterScript(bundle: string, skinId: string, requestedVariant?: string): string {
  ...
  const activationProfiles: Record<string, { storageKey: string; defaultValue: string; variants?: string[] }> = {
    'caoyiwei850.dsh-client-ui-skins': { storageKey: 'dsh-skins.active', defaultValue: 'skin-mint-clean', variants: ['skin-ocean-deep', 'skin-sakura-pink', 'skin-mint-clean', 'skin-amber-glow'] },
    'zhijun-dai.catppuccin': { storageKey: 'dsh-catppuccin:skin', defaultValue: 'catppuccin-mocha', variants: ['catppuccin-latte', 'catppuccin-frappe', 'catppuccin-macchiato', 'catppuccin-mocha'] },
    'jungeer.dsh-theme-stardew': { storageKey: 'dsh.ui-stardew.enabled', defaultValue: 'on' },
  }
  const profile = activationProfiles[skinId]
  if (requestedVariant !== undefined && !/^[a-z0-9][a-z0-9._-]{1,120}$/i.test(requestedVariant)) throw new Error('invalid skin variant for ' + skinId)
  if (requestedVariant !== undefined && profile?.variants !== undefined && !profile.variants.includes(requestedVariant)) throw new Error('unsupported skin variant for ' + skinId + ': ' + requestedVariant)
  ...
  const activationDefault = JSON.stringify(profile === undefined ? null : [profile.storageKey, requestedVariant ?? profile.defaultValue])
  const forceActivationDefault = requestedVariant !== undefined
  const protocolBase = 'dsh-skin://' + skinId
  const assetPathPattern = /(["'])\/(?!\/)([a-z0-9._-]+\/(?:[^"']*?\.(?:png|jpe?g|webp|gif|svg|avif|woff2?|ttf)))(["'])/gi
  const visualAssetUrls: string[] = []
  const rewrittenAssets = bundle.replace(assetPathPattern, (_match, quote: string, path: string) => { const url = protocolBase + '/' + path; if (/\.(?:png|jpe?g|webp|gif|svg|avif)$/i.test(path)) visualAssetUrls.push(url); return quote + url + quote })
  ...
  const adaptedBundle = skinId === 'd-dev0101.open-sea-skin'
    ? rewrittenAssets.replaceAll("'/open-sea-skin/' + path", "'dsh-skin://d-dev0101.open-sea-skin/' + path")
    : rewrittenAssets
  return `(async () => {
  'use strict';
  try {
  ...
  const backgroundAssetUrl = ${backgroundAssetUrl};
  const supportsCustomBackground = ${supportsCustomBackground};
  const customBackgroundStorageKey = ${customBackgroundStorageKey};
  const legacyCustomBackgroundStorageKey = ${legacyCustomBackgroundStorageKey};
...
${adaptedBundle}
  ...
  window.addEventListener('message',sessionMessage);disposers.push(()=>window.removeEventListener('message',sessionMessage));
  ...
  const context = { theme, slots, locale, sessions, remote, reflect, effect(register) { try { const result = register(); if (result && typeof result.then === 'function') { result.then(disposer => { if (typeof disposer === 'function') disposers.push(disposer); }).catch(error => console.warn('[dsh skin optional effect]', error)); return noop; } if (typeof result === 'function') disposers.push(result); return result; } catch (error) { console.warn('[dsh skin optional effect]', error); return noop; } }, on, once(name, listener) { let off = noop; off = on(name, (...args) => { off(); listener(...args); }); return off; }, timeout: timer.timeout, interval: timer.interval, get(name) { if (name === 'slots') return slots; if (name === 'timer') return timer; if (name === 'theme') return theme; if (name === 'locale') return locale; if (name === 'sessions') return sessions; if (name === 'remote') return remote; if (name === 'remote.whaleCompanion') return whaleCompanion || undefined; if (name === 'reflect') return reflect; return undefined; }, plugin: noop };
...
export function createSkinDisposerScript(): string {
...
export function createSkinMarketInjectorScript(): string {
  return String.raw`(() => {
  'use strict';
...
export const getSkinMarketInjectorScript = createSkinMarketInjectorScript
