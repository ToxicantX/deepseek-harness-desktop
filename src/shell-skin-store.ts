...
export interface ShellSkinState { activeSkinId:string|null; installed:Record<string,{version:string;directory:string;clientPath:string;compatible:boolean;error?:string}> }
...
export class ShellSkinStore {
  ...
  list(){ return {skins:this.catalogValue.map(s=>({...s,screenshots:s.screenshots?.map((_,index)=>String(index)),runtime:this.runtime(s.id)})),activeSkinId:this.stateValue.activeSkinId} }
