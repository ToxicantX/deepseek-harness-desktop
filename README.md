...
通过 **文件 → 个性化皮肤 → 选择本地图片...** 选择 PNG、JPEG 或 WebP 静态图片。Shell 会拒绝超过 8 MB 或尺寸超过 4096×4096 的文件，将有效图片按最长边 512 像素规范化为 PNG，并保存到应用 `userData` 下的 `desktop-pet-skin.png`；图片路径不会发送给宠物 renderer 或 DSH Runtime。

选择 **文件 → 个性化皮肤 → 恢复默认皮肤** 会删除自定义文件并立即恢复内置图标。自定义皮肤会在 Shell 重启和宠物 renderer 恢复后继续生效。
