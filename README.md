...
通过 **文件 → 个性化皮肤 → 选择静态图片或动态 GIF...** 选择 PNG、JPEG、WebP 静态图片或动态 GIF。静态图片上限为 8 MB、4096×4096，Shell 会按最长边 512 像素规范化为 PNG。动态 GIF 会保留原始帧，限制为 2 MB、最长边 512 像素、2–120 帧及总计 16777216 帧像素，并保存到应用 `userData` 下的 `desktop-pet-skin.gif`。系统启用“减少动态效果”时会显示主进程生成的首帧 PNG；原始选择路径不会发送给宠物 renderer 或 DSH Runtime。

选择 **文件 → 个性化皮肤 → 恢复默认皮肤** 会删除静态和动态自定义文件并立即恢复内置二次元角色。自定义皮肤会在 Shell 重启和宠物 renderer 恢复后继续生效。
